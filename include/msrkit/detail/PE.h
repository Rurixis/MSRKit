...
namespace MSRK::detail
{
    ...
    class M_PE final
    {
        ...
        friend bool  MSRK::INIT(const wchar_t*, HANDLE);
        
        private:
            static std::size_t GET_IMAGE_SIZE(const char* IMAGE_PATH)
            {
                const HANDLE FILE_HANDLE = CreateFileA(
                    IMAGE_PATH, GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING, 0, nullptr
                );
                if (FILE_HANDLE == INVALID_HANDLE_VALUE) return 0;

                IMAGE_DOS_HEADER DOS_HEADER{};
                DWORD BYTES_READ = 0;
                if (!ReadFile(FILE_HANDLE, &DOS_HEADER, sizeof(DOS_HEADER), &BYTES_READ, nullptr) ||
                ...
                SetFilePointer(FILE_HANDLE, DOS_HEADER.e_lfanew, nullptr, FILE_BEGIN);
                IMAGE_NT_HEADERS64 NT_HEADER{};
                if (!ReadFile(FILE_HANDLE, &NT_HEADER, sizeof(NT_HEADER), &BYTES_READ, nullptr) ||
                    BYTES_READ != sizeof(NT_HEADER) || NT_HEADER.Signature != IMAGE_NT_SIGNATURE)
                {
                    CloseHandle(FILE_HANDLE);
                    return 0;
                }
                ...
                CloseHandle(FILE_HANDLE);
                return (std::size_t)NT_HEADER.OptionalHeader.SizeOfImage;
            }
            ...
            static std::uintptr_t GET_NTOSKRNL_BASE()
            {
                ...
                if ( !EnumDeviceDrivers(DRIVER_LIST, sizeof(DRIVER_LIST), &BYTES_NEEDED) )
                {
                    return 0;
                }
            ...
            static bool FIX_IMPORT(std::uint8_t* IMAGE_BASE)
            {
                ...
                if (DRIVER_COUNT > 1024) DRIVER_COUNT = 1024;
